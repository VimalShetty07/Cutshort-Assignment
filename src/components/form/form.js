...
function Form() {
  const { page, setPage, titles, formData, setFormData } =
    useGlobalFormValues();
    ...
    <div className="main-app-container">
      <div className="form-container">
        <div className="head-name">
          ...
          <h2 className="brand-name">Eden</h2>
        </div>
        <div>
          <ProgrssBar />
        </div>
        <div>
          ...
          <button
            className="space-btn"
            onClick={page === titles.length - 1 ? handleUser : handelSPace}
          >{`${
            page === titles.length - 1 ? "Launch Eden" : "Create Workspace"
          }`}</button>
        </div>
      </div>
    </div>
